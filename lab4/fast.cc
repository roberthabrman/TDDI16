...
//	sort(points.begin()+2, points.end(), PolarSorter(points.at(0)));
...
//pseudokod:
int i = 0;
sort(points.begin() + i, points.end(), PolarSorter(points.at(i)));

cout << points.at(i).slopeTo(points.at(i + 1)) << "       " << points.at(i).slopeTo(points.at(i + 3));
cout << points.at(i).slopeTo(points.at(i + 2)) << "       " << points.at(i).slopeTo(points.at(i + 4));
//cout << points.at(i).slopeTo(points.at(i + 1)) << "       " << points.at(i).slopeTo(points.at(i + 3));
//cout << points.at(i).slopeTo(points.at(i + 1)) << "       " << points.at(i).slopeTo(points.at(i + 3));
...
/*
	for (int i{0} ; i < N - 3 ; ++i)
	{
		...
		if ( points.begin() + i + 3 < points.end())
		{
			//kommer hit
			/*			//fel villkor?
			if (points.at(i).slopeTo(points.at(i+1)) - points.at(i).slopeTo(points.at(i+3)) == 0)
			{
				cout << points.at(i).slopeTo(points.at(i+1)) << "      " << points.at(i).slopeTo(points.at(i+3)) << endl;
			}
			*/ /*
			cout << points.at(i).slopeTo(points.at(i + 1)) << "      " << points.at(i).slopeTo(points.at(i + 3)) << endl;
			if (points.at(i).slopeTo(points.at(i + 1)) == points.at(i).slopeTo(points.at(i + 3)) )
			{
				
				//skriver ut punk a-b ,a-c,a-d
				render_line(screen, points.at(i), points.at(i + 1));
				render_line(screen, points.at(i), points.at(i + 2));
				render_line(screen, points.at(i), points.at(i + 3));
			}
	...
	*/
	...
	/*
	int z = 0;
		for(vector<Point>::iterator x ; x != points.end() ; x++)
		{
